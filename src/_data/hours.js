...
/**
 * Get business hours from configured data source
 */
module.exports = async function() {
  ...
  try {
    return await adapter.getHours();
  } catch (error) {
    console.error(`❌ Adapter error (${dataSource}):`, error.message);
    // Return user-friendly error message for fallback UI
    const userFriendlyError = error.message || 'Unable to load business hours at this time';
    return {
      items: [],
